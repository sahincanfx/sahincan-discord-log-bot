...
logs(client)
client.log = "Channel ID";
client.Token = "Bot Token";

const send = async function send(type, content, imageURL) {i
const embed = new MessageEmbed().setColor("RANDOM").setTimestamp().setTitle(type).setDescription(content).setFooter("Sahincan ❤️ Reawen")
...
client.login(client.Token).then(qwe => console.log(`${client.user.tag} online!`)).catch(hata => console.error("Bot can't connected!"));
...
client.on("ready", () => {
send("Bot online!", `Bot connected!`);
console.log("Bot connected!");
client.user.setPresence({ activity: { type: "PLAYING", name: `Sahincan ❤️ Reawen`}, status: 'online' });
})
